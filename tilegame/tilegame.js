...
function make_tile(data, col, row) {
    ...
    if (JSON.stringify(data) != JSON.stringify(naked_data)) {
        tile.classList.add(`top-${classes[data.top - 1]}`)
        tile.classList.add(`bottom-${classes[data.bottom - 1]}`)
        tile.classList.add(`right-${classes[data.right - 1]}`)
        tile.classList.add(`left-${classes[data.left - 1]}`)
    }
